...
test_metric_brief_multiline if {
    # Should pass: multiline metric brief ends with period
    count(before_resolution.deny) == 0 with input as {"groups": [
        {"id": "metric.test", "type": "metric", "brief": "This is a multiline\nmetric brief.", "stability": "development"}
    ]}
    
    # Should fail: multiline metric brief doesn't end with period
    count(before_resolution.deny) > 0 with input as {"groups": [
        {"id": "metric.test2", "type": "metric", "brief": "This is a multiline\nmetric brief without period", "stability": "development"}
    ]}
}
